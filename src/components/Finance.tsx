...
import { Transaction, Client, Supplier, ImportedInvoice } from '@/src/types';
...
export default function Finance({ 
  ...
  const [activeTab, setActiveTab] = React.useState<'geral' | 'notas'>('geral');
  ...
  return (
    ...
    >
      ...
      {/* Tabs */}
      <div className="flex gap-1 p-1 bg-[#f5f2fb] rounded-2xl w-fit">
        <button 
          onClick={() => setActiveTab('geral')}
          className={cn(
            "px-6 py-2.5 rounded-xl text-sm font-bold transition-all",
            activeTab === 'geral' ? "bg-white text-[#000666] shadow-sm" : "text-slate-500 hover:text-[#000666]"
          )}
        >
          Visão Geral
        </button>
        <button 
          ...
          className={cn(
            ...
            activeTab === 'notas' ? "bg-white text-[#000666] shadow-sm" : "text-slate-500 hover:text-[#000666]"
          )}
        >
          <FileText size={16} />
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'geral' ? (
          ...
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
          >
            <ImportedInvoicesList invoices={importedInvoices} />
          </motion.div>
        )}
      </AnimatePresence>
